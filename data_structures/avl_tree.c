...
/* add `data` to tree */
int avl_insert( avl_handle* handle, void* data )
{
    avl_node* an;
...
#ifdef __main_avl_tree__  // if __name__ == "__main__": ...
...
void traverse(void* a, int level)
{
    if( a )
        printf("%*d\n", level*2, *((int*)a));
    else
...
int main( void )
{
    ...
    ah = avl_init( sizeof(int), compare );
    ...
    do
    {
        ...
        if( avl_insert(ah, &in) )
            ...
            printf( "not add\n" );
        avl_traverse( ah, MID_TRS, traverse );
    }
    while( in );
    do
    {
